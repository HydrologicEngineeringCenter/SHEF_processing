...
<details>
  <summary>What is SHEF?</summary>
  <div class="boxed-content">
  <p>
  SHEF is an acronym for Standard Hydrometeorologic Exhchange Format, created in the early 1980s by
  NOAA/NWS, with major participation from USACE, USGS, and other agencies, to provide a standard
  format for exchanging hydromet observations and forecasts that is both human readable and machine
  parsable.
  </p><p>
  ...
  <a href="https://www.google.com/url?sa=t&rct=j&q=&esrc=s&source=web&cd=&ved=2ahUKEwiqkISqkvqQAxXyQjABHdeoE9QQFnoECBkQAQ&url=https%3A%2F%2Fwww.weather.gov%2Fmedia%2Fmdl%2FSHEF_CodeManual_5July2012.pdf&usg=AOvVaw0k3t5QKxjiMX1oPJXSOR3K&opi=89978449">
  here</a>.
  </p><p>
...
So the SHEF parser is really just a fancy text format converter. However, it is a necessary one because
while SHEF text must adhere to a standard, in order to meet the goal of human readability the standard is very flexible with regard to:
* unit system
* time zone
* use of whitespace
...
Having a dedicated parser to convert SHEF text into a not-so-human-readable format that is much simpler to
parse by machine allows downstream programs to employ much simpler parsing code.

In this package SHEF parser is the [`shef_parser`](shef/shef_parser.html) module.
...
<details>
  ...
  The reference SHEF parser, <code>shefit</code> - a Fortran program developed by the NWS -
  also allows outputting to a binary format in addition to the text output formats 1 and 2. This package
  doesn't support generating or reading the binary format.
  </div>
</details>
...
In this package, loaders are not stand-alone programs but are sub-modules written to an API that allows
[`shef_parser`](shef/shef_parser.html) to pass its output directly to them to be loaded to the desired data store. 
To tell [`shef_parser`](shef/shef_parser.html) to use a specific loader you use the `--loader` command
line switch follwed by loader-specific arguments. The arguments required by each loader are specified on the
command line in one or more bracketed text strings (e.g., `[arg1][arg2]...`) and can be displayed by 
running `python shef/shef_parser.py --description`.

In the event that one has on hand not the SHEF text but parsed text, [`shef_parser`](shef/shef_parser.html) can pass the 
processed text through to an attached loader by using the `--processed` command line argument.

Information on how loaders interact with [`shef_parser`](shef/shef_parser.html) and the API they are required
to implement is detailed on the [`loaders`](shef/loaders.html) submodule page.

## Exporting/Unloading
As described on the [`loaders`](shef/loaders.html) submodule page, each loader module is required to have a public boolean variable
named `can_unload` that specifies whether its loader class contains an `unload()` method. If provided, the `unload()` method
reads time series data on the loader's input and generates one or more SHEF messages for each time series read.
Typically, one `.E` message or multiple `.A` messages for each time series depending on whether the time series has
a regular or irregular interval.

While a loader's loading input (parsed SHEF text) can be generated by [`shef_parser`](shef/shef_parser.html), there is no way
for it to generate a loader's unloading input (loader-specific-formatted time series). This is the role of [`exporters`](shef/exporters.html).
This requires each loader to have its own exporter.

The role of an exporter is to retrieve time series from a data store and write them to the loader's input in the correct format for unloading.
...
While exporters may define their own methods to allow them to be exected from the command line, there is no requirement 
for them to do so. The API they are required to implement is detailed on the [`exporters`](shef/exporters.html) submodule page.
