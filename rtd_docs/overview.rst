...
method reads time series data from the loader's input and generates one or more SHEF messages for each time series read.
...
requires each loader to have its own exporter. [1]_
...
.. [1] If you have *pre-extracted* time series from the data store in the loader's expected format, you can generate
   SHEF text from it by running ``run_shef_parser -i <pre-extracted-data> --loader <loader-spec> --unload`` and dont' need
   to use an exporter
